...
`master`ブランチは開発に使用され、`stable`ブランチはリリースに使用されます。

1. すべてのテストが通ることを確認する
2. `stable`を対象としたプルリクエストを作成する
3. プルリクエストをレビューして承認する
4. 承認されたプルリクエストを`stable`にマージする
5. `stable`上ですべてのテストが通ることを確認する
6. `stable`上でタグを作成し、そのタグからリリースを作成する
